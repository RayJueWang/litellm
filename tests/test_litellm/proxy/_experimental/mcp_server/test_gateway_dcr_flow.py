...
@pytest.mark.asyncio
async def test_curl_suggestion_is_double_quoted_for_a_normal_loopback_url():
    """cmd.exe does not honor single quotes, so the suggested command must be the
    double-quoted form that every mainstream shell treats as one inert argument."""
    _, response = await _complete(LOOPBACK_REDIRECT_URI, delivery="manual")
    body = response.body.decode()
    assert 'curl "http://localhost:3118/callback?code=' in body
    assert "curl '" not in body


@pytest.mark.asyncio
async def test_curl_suggestion_omitted_for_shell_hostile_redirect_uri():
    """Veria review finding on the LIT-4863 change: a registered redirect URI carrying
    shell metacharacters must not yield a suggested command at all; the copyable URL box
    stays, the command line goes."""
    hostile_uri = "http://127.0.0.1:9/cb$(whoami)&calc& rem x"
    _, response = await _complete(hostile_uri, delivery="manual")
    assert response.status_code == 200
    body = response.body.decode()
    assert "curl" not in body
    assert 'value="' in body
    assert "$(whoami)" in html.unescape(body)
