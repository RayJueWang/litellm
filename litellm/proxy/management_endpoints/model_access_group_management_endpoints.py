...
def _raise_http_if_write_dropped_on_reload(
    reload_succeeded: bool,
    written_models: Sequence[tuple[str, Mapping[str, object] | None]],
    access_group: str,
) -> None:
    """Same contract as the model-write endpoints, expressed through this file's
    HTTPException error convention: the access-group rows are saved either way; this
    reports when the handling pod is not serving them after the reload it triggered."""
    if not reload_succeeded:
        raise HTTPException(
            status_code=500,
            detail={
                "error": (
                    f"Access group '{access_group}' was saved to the database, but this pod failed to reload "
                    "models afterwards, so the change is not being served by this pod yet. Other pods reload "
                    "on their own interval. Check server logs for 'ProxyConfig:add_deployment' for the cause."
                )
            },
        )
    ...
    raise HTTPException(
        ...
        detail={
            "error": (
                f"Access group '{access_group}' was saved to the database, but the model id(s) {list(dropped)} "
                "failed to load into this pod's live router after the reload and are not being served by this "
                "pod. Other pods reload on their own interval. Check server logs for 'Error upserting deployment' "
                "for the cause."
            )
        },
    )
...
async def create_model_group(
    ...
    try:
        ...
        _raise_http_if_write_dropped_on_reload(
