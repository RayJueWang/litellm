...
_SHELL_SAFE_URL_CHARS = frozenset(string.ascii_letters + string.digits + ":/?=&._~%+-")
"""Characters of a callback URL that stay inert inside a double-quoted string on every
mainstream shell (POSIX sh/bash/zsh, Windows cmd.exe, PowerShell). Excludes the double
quote itself and every trigger some shell still honors inside double quotes (``$``,
backtick, backslash, ``!``) plus all whitespace. A POSIX-only ``shlex.quote`` is not
enough to SUGGEST a command: cmd.exe does not treat single quotes as quoting, so a
hostile registered redirect URI would inject there. A URL outside this set simply gets
no suggested command; the copyable URL itself remains."""
...
def _manual_delivery_response(callback_url: str) -> Response:
    ...
    the client's prompt, or fetch from that machine's terminal). Served no-store because
    the body holds a live single-use code; every rendered value is client-influenced, so
    the URL is HTML-escaped and a curl suggestion renders only when the URL is
    cross-shell inert per ``_SHELL_SAFE_URL_CHARS``."""
    safe_url = html.escape(callback_url, quote=True)
    minutes = MANUAL_DELIVERY_AUTH_CODE_TTL_SECONDS // 60
    curl_block = (
        f'<p>or fetch it from that machine\'s terminal:</p><p><code>curl "{safe_url}"</code></p>'
        if all(c in _SHELL_SAFE_URL_CHARS for c in callback_url)
        else ""
    )
    body = (
        ...
        " the client's prompt (Claude Code accepts the pasted callback URL):</p>"
